...
/*--------------------------------------------------------------------- 
 * Method: sr_destroy_interface(..)
 * Scope: Global
 *
 * remove and free an interface from the router's list
 *
 *---------------------------------------------------------------------*/

void sr_destroy_interface(struct sr_instance* sr)
{
    struct sr_if* curr = sr->if_list;
    struct sr_if* prev;
    while(curr){
         printf("freeing previous interfaces %lu\n", sizeof(*(curr)));
         prev = curr;
	 curr = curr->next;
         free(prev);
    }
