 ...
 *
 *---------------------------------------------------------------------*/
void sr_destory_rt(struct sr_instance* sr){
    struct sr_rt* next;
    while (sr->routing_table){
        printf("freeing previous routing table %lu\n", sizeof(*(sr->routing_table)));
	next = sr->routing_table->next;
        free(sr->routing_table);
